...
def eval_once(saver, mse_op, summary_writer, summary_op):
...
def evaluate():
    with tf.Graph().as_default() as g:
        ...
        reality = tf.add(tf.multiply(reality, 184), 282)
        
        logits = cnn_contrast.cnn_model_1(prediction)
        #logits = prediction
        logits = tf.add(tf.multiply(logits, 182), 282)
        ...
        while True:
            eval_once(saver, mse, summary_writer, summary_op)
            if FLAGS.run_once:
                break
