...
def cnn_low_resolution(input_data):
    ...
    pool2 = ops.max_pooling(conv2, 2, 2, padding='VALID', name='l_pool2')
...
def cnn_high_resolution(input_data):
    ...
    pool2 = ops.max_pooling(conv2, 2, 2, padding='VALID', name='h_pool2')

    with tf.variable_scope('h_conv3') as scope:
        conv3= ops.conv2d(pool2, 3, 7, 32, 32, padding='VALID', name=scope.name)
