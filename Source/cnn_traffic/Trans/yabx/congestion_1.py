...
def test():
    with tf.Graph().as_default() as g:
        ...
        ltoday, mtoday, htoday, tomorrow, today_max, today_min, tomorrow_max, tomorrow_min, today = rec.test_inputs(
        ...
        try:
            ...
            while step < num_epoch:
                ...
                # matlab.save_matrix(os.path.join(pred_log_dir, str(step+1)+'.mat'), pred_matrix, 'pred_m')
                # matlab.save_matrix(os.path.join(real_log_dir, str(step+1)+'.mat'), real_matrix, 'real_m')
                # matlab.save_matrix(os.path.join(today_log_dir, str(step+1)+'.mat'), today_matrix, 'today_m')
                # matlab.save_matrix(os.path.join(pred_fix_log_dir, str(step+1)+'.mat'), pred_fix, 'fix_m')
                matlab.save_matrix(os.path.join(delta_pt_dir, str(step+1)+'.mat'), pt_delta, 'pt_delta')
                matlab.save_matrix(os.path.join(delta_fr_dir, str(step+1)+'.mat'), fr_delta, 'fr_delta')
                matlab.save_matrix(os.path.join(delta_pr_dir, str(step+1)+'.mat'), pr_delta, 'pr_delta')
                ...
                rw_mre = losses.np_mre(today_matrix, real_matrix)
                rw_list.append(rw_mre)
                if mre<fix_mre:
