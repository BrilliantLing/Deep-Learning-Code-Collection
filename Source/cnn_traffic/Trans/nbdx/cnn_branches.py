...
def cnn_with_branch(low_data, mid_data, high_data, out_channels, batch_size, is_train=True):
    ...
    with tf.variable_scope('l_conv3') as scope:
        lconv3= ops.conv2d(lconv2, 2, 3, 32, 64, padding='VALID', name=scope.name)
    lpool3 = ops.max_pooling(lconv3, 2, 2, padding='VALID', name='l_pool3')
    ...
    with tf.variable_scope('m_conv3') as scope:
        mconv3 = ops.conv2d(mconv2, 2, 3, 32, 64, padding='VALID', name=scope.name)
    ...
    with tf.variable_scope('h_conv3') as scope:
        hconv3 = ops.conv2d(hconv2, 2, 5, 32, 64, padding='VALID', name=scope.name)
    hpool3 = ops.max_pooling(hconv3, 2, 6, [1,2,6,1],padding='VALID',name='h_pool3')
